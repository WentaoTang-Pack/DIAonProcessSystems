...
for i = 1:size(v_list,2)
    ...
    [~,D1_k4(i),D2_k4(i)] = f_obj_ksparse(Sigma_xixi,Sigma_yy,Sigma_aa_temp,lambda,A,B,K,L,C,Vd,Vn);
    [P_d_k4(i),P_f_k4(i)] = f_AttackDetection(Sigma_yy,Sigma_aa_temp);
end   
[~,~,~,Sigma_hatxahatxa,~,~] = cov_matrix(A,B,K,L,C,Vd,Vn,Sigma_aa_temp);



plot(D2_k4,-D1_k4,'-d', 'LineWidth', 1.5, 'MarkerSize', 6, 'Color', [0.5 0.5 0.5]), hold on;
save('D1_k4','D1_k4')
save('D2_k4','D2_k4')
save('P_d_k4','P_d_k4')
save('P_f_k4','P_f_k4')
