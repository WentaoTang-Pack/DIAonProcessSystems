...
for k = 1:k_max-1
    ...
    [~,D1(k),D2(k)] = f_obj_ksparse(Sigma_xixi,Sigma_yy,Sigma_aa,lambda,A,B,K,L,C,Vd,Vn);
    A_set = [A_set,ind];
    [P_d(k),P_f(k)] = f_AttackDetection(Sigma_yy,Sigma_aa);
end
sensor_candi = setdiff(measure_ind,A_set);
[ind] = f_bestSensor_ksparse(sensor_candi,Sigma_yy,M2,Sigma_aa,Sigma_xixi,A,B,K,L,C,Vd,Vn,lambda);  
...
for i = 1:size(v_list,2)
    ...
    [~,D1_k1(i),D2_k1(i)] = f_obj_ksparse(Sigma_xixi,Sigma_yy,Sigma_aa_temp,lambda,A,B,K,L,C,Vd,Vn);
    [P_d_k1(i),P_f_k1(i)] = f_AttackDetection(Sigma_yy,Sigma_aa_temp);
end   
plot(D2_k1,-D1_k1,'-o', 'LineWidth', 1.5, 'MarkerSize', 6, 'Color', [1 0 0]), hold on;
save('D1_k1','D1_k1')
save('D2_k1','D2_k1')
save('P_d_k1','P_d_k1')
save('P_f_k1','P_f_k1')
